// Letakkan di: app/api/laporan/stok_periode/export/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    // E=Stok Awal Kemasan, F=Stok Awal PCS, G=Stok Awal Total
    // H=Terjual Kemasan, I=Terjual PCS, J=Terjual Total
    // K=Stok Akhir Kemasan, L=Stok Akhir PCS, M=Stok Akhir Total
    // N=Nilai Terjual, O=Modal Terjual, P=Laba Kotor
    ...
    ws.columns = [
      ...
      { width: 20 }, // C Supplier
      { width: 14 }, // D Kemasan
      { width: 12 }, // E Stok Awal Kemasan
      { width: 11 }, // F Stok Awal PCS
      { width: 12 }, // G Stok Awal Total
      { width: 12 }, // H Terjual Kemasan
      { width: 11 }, // I Terjual PCS
      { width: 12 }, // J Terjual Total
      { width: 12 }, // K Stok Akhir Kemasan
      { width: 11 }, // L Stok Akhir PCS
      { width: 12 }, // M Stok Akhir Total
      { width: 16 }, // N Nilai Terjual
      { width: 16 }, // O Modal Terjual
      { width: 16 }, // P Laba Kotor
    ];
    ...
    const grpAwal = ws.getCell(`E${GRP_ROW}`);
    grpAwal.value = "STOK AWAL (ESTIMASI)";
    applyStyle(grpAwal, grpStyle("FF00695C"));

    // Grup: Terjual (H-J)
    ws.mergeCells(`H${GRP_ROW}:J${GRP_ROW}`);
    const grpTerjual = ws.getCell(`H${GRP_ROW}`);
    grpTerjual.value = `TERJUAL (${periodeLabel})`;
    applyStyle(grpTerjual, grpStyle("FF1565C0"));

    // Grup: Stok Akhir (K-M)
    ws.mergeCells(`K${GRP_ROW}:M${GRP_ROW}`);
    const grpAkhir = ws.getCell(`K${GRP_ROW}`);
    grpAkhir.value = "STOK AKHIR (SEKARANG)";
    applyStyle(grpAkhir, grpStyle("FF6A1B9A"));

    // Grup: Nilai (N-P)
    ws.mergeCells(`N${GRP_ROW}:P${GRP_ROW}`);
    const grpNilai = ws.getCell(`N${GRP_ROW}`);
    ...
    const subHeaders: { col: string; label: string; argb: string }[] = [
      { col: "E", label: "Kemasan", argb: "FF00695C" },
      { col: "F", label: "PCS", argb: "FF00695C" },
      { col: "G", label: "Total", argb: "FF00695C" },
      { col: "H", label: "Kemasan", argb: "FF1565C0" },
      { col: "I", label: "PCS", argb: "FF1565C0" },
      { col: "J", label: "Total", argb: "FF1565C0" },
      { col: "K", label: "Kemasan", argb: "FF6A1B9A" },
      { col: "L", label: "PCS", argb: "FF6A1B9A" },
      { col: "M", label: "Total", argb: "FF6A1B9A" },
      { col: "N", label: "Nilai Jual", argb: "FFB71C1C" },
      { col: "O", label: "Modal", argb: "FFB71C1C" },
      { col: "P", label: "Laba Kotor", argb: "FFB71C1C" },
    ];
    ...
    data.forEach((row, idx) => {
      ...
      r.eachCell((cell, col) => {
        ...
        // Stok Awal (E=5, F=6, G=7) → tint hijau
        if ([5, 6, 7].includes(col)) {
          ...
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: isOdd ? "FFE8F5E9" : "FFF1F8F2" },
          };
        }
        // Terjual (H=8, I=9, J=10) → tint biru
        if ([8, 9, 10].includes(col)) {
          ...
          if (col === 10 && row.totalTerjual === 0) {
            cell.font = { color: { argb: "FFBDBDBD" } };
          }
        }
        // Stok Akhir (K=11, L=12, M=13) → tint ungu
        ...
        // Nilai (N=14, O=15, P=16) → rupiah
        if ([14, 15].includes(col)) {
